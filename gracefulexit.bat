...
if [%1]==[] (
	echo Usage: gracefulexit {sleep^|switch^|lock^|logoff^|pause} task1 task2 ...
	exit /b 1
)
...
@rem Some tasks (e.g. Quicken) will not close when minimized
	...
	for /f "tokens=1,11 delims=: " %%G in ('taskkill /fi "USERNAME eq %USERNAME%" /im %pname%') do (
		if %%G==SUCCESS (
			...
			powershell -Command "Wait-Process -Id !pid! -Timeout 10" | find /i "time-out"
			if %ErrorLevel% equ 0 (
				echo %pname% PID !pid! did not respond to taskkill, retrying ...
				goto KILLTASKS
			)
		)
	)
